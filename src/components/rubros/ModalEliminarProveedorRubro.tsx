  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth sx={{ '& .MuiDialog-paper': { borderRadius: 3, overflow: 'hidden' } }}>
      ...
      <DialogContent sx={{ mt: 4, p: 5, pt: 4, bgcolor: '#fff' }}>
        <Box display="flex" flexDirection="column" gap={3}>
          <Box sx={{ p: 3, borderRadius: 2, border: '2px solid #ffebee', bgcolor: '#fafafa', display: 'flex', alignItems: 'center', gap: 2 }}>
            ...
            <Box>
              <Typography variant="body1" fontWeight={600} color="#d32f2f">¿Está seguro que quiere eliminar este proveedor del rubro?</Typography>
      ...
      <DialogActions sx={{ p: 4, pt: 2, bgcolor: '#fafafa', borderTop: '1px solid #eee' }}>
        ...
        <Button onClick={onConfirm} disabled={textoConfirmacion !== 'ELIMINAR'} sx={{ textTransform: 'none', borderRadius: 2, px: 4, bgcolor: '#d32f2f', color: 'white', fontWeight: 600, '&:hover': { bgcolor: '#b71c1c' }, '&:disabled': { bgcolor: '#ccc', color: '#999' } }}>⚠️ ELIMINAR PROVEEDOR</Button>
      </DialogActions>
    </Dialog>
  );
}
