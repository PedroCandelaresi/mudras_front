...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const url = join(base, `/permissions${queryString ? `?${queryString}` : ''}`);
    ...
    console.error('Error en /api/permissions GET:', error);
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const res = await fetch(join(base, '/permissions'), {
      method: 'POST',
      headers: {
    ...
    console.error('Error en /api/permissions POST:', error);
    return new NextResponse('Error interno del servidor', { status: 500 });
  }
}
